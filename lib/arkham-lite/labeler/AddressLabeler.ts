import { ContractLabel } from '../types';

export class AddressLabeler {
  private staticLabels: Map<string, ContractLabel>;
  private apiCache: Map<string, ContractLabel>;
  ...
  private initializeStaticLabels() {
  ...
  private addStaticLabel(address: string, label: string, type: ContractLabel['type']) {
    this.staticLabels.set(address.toLowerCase(), { address: address.toLowerCase(), label, type });
  }

  async getLabel(address: string, chain: string = 'ethereum'): Promise<string | undefined> {
  ...
  private async fetchFromBlockExplorer(address: string, chain: string): Promise<ContractLabel | null> {
    const apiUrls: Record<string, { url: string; key?: string }> = {
      ethereum: {
  ...
  async labelAddresses(addresses: string[], chain: string = 'ethereum'): Promise<Record<string, string | undefined>> {
