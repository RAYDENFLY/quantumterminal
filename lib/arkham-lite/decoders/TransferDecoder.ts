...
import { TransferEvent, TokenMetadata, LogData } from '../types';
...
export class TransferDecoder {
  ...
  async decodeTransferLog(log: LogData, provider: ethers.JsonRpcProvider): Promise<TransferEvent | null> {
    try {
      ...
      const amount = parseFloat(ethers.formatUnits(value, tokenMetadata.decimals));
  ...
  private async getTokenMetadata(tokenAddress: string, provider: ethers.JsonRpcProvider): Promise<TokenMetadata | null> {
    try {
  ...
  async decodeBatch(logs: LogData[], provider: ethers.JsonRpcProvider): Promise<TransferEvent[]> {
    const decodedEvents: TransferEvent[] = [];
    ...
    for (let i = 0; i < logs.length; i += batchSize) {
      ...
      decodedEvents.push(...batchResults.filter((event): event is TransferEvent => event !== null));
    }
