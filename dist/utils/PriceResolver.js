...
class PriceResolver {
    ...
    async fetchFromCoinGecko(tokenAddress, chain) {
        ...
        if (!platform)
            return null;
        try {
            const response = await fetch(`https://api.coingecko.com/api/v3/simple/token_price/${platform}?contract_addresses=${tokenAddress}&vs_currencies=usd`);
            if (!response.ok)
                return null;
    ...
    async fetchFromDEX(tokenAddress, chain) {
    ...
    async getBatchPrices(tokenAddresses, chain = 'ethereum') {
        const results = {};
    ...
    async getPriceData(tokenAddress, symbol, chain = 'ethereum') {
